...
import { CongestionInfographicsMobilePage } from '../congestion-infographics-mobile/congestion-infographics-mobile';
...
@ViewChild(Content) content: Content;
  ...
  constructor(public navCtrl: NavController, public navParams: NavParams, public menu: MenuController, public platform: Platform) {
  	...
  	this.subPages = [
  			UrgentCarePage, 			// 0
  			AllergiesMobilePage,		// 1
  			ColdMobilePage,				// 2
  			CongestionInfographicsMobilePage,	// 3
  		];
  ...
  toggleFYIDlg(b: boolean) {
    if (b)
    {
		...
		this.FYIDlg.top = (this.MyContent.screenHeight - this.FYIDlg.height) / 2 + scrollPos;
    }
    this.FYIDlg.show = b;
  }
  ...
  togglePage(ind: number) {
  	if (ind == 2)
  	{
	  	...
	  	if (this.symptoms[5] == true)
	  		this.mode = 1;
	  	else if (this.symptoms[4] == true)
	  		this.mode = 2;
	  	else if (this.symptoms[0] == true)
	  		this.mode = 3;
	  	else if (this.symptoms[2] == true)
	  		this.mode = 4;
