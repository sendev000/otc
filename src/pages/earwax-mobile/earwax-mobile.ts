...
import { EarwaxInfographicMobilePage } from '../earwax-infographic-mobile/earwax-infographic-mobile';
...
@ViewChild(Content) content: Content;
  ...
  toggleExp(ind: number){
  	this.exp[6] = false;
  }
  toggleAllExp(b: boolean){
  	for (let i=0;i<6;i++)
  		this.exp[i] = b;
  }
  ...
  togglePage(ind: number) {
  	if (ind == 2)
  	{
	  	...
	  	if (this.exp[6] == true)
	  		this.mode = 1;
	  	else if (this.exp[5] == true)
	  		this.mode = 2;
	  	else if (this.exp[0] == true)
	  		this.mode = 3;
	  	else if (trueCount > 0)
	  		this.mode = 4;
	  	else{
